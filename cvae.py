...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', '--maxiter', type=int, default=1000, help='number of epochs to train (default: 10)')
    ...
    parser.add_argument('--dir', help='dataset directory', default='C:/Users/Secil/Desktop/Hattusha/data')
    ...
    parser.add_argument('--rating', help='feed input as rating', action='store_false')
    parser.add_argument('--save', help='save model', action='store_false')
    parser.add_argument('--load', help='load model, 1 for fvae and 2 for cvae', type=int, default=1)
    ...
    if args.rating:  # feed in with rating
        ...
        #kf = StratifiedKFold(data_set[:, 2], n_splits = 5, shuffle=True, random_state=rs)
        kf = KFold(n_splits=10, random_state=rs, shuffle=True)
        for train_index, test_index in kf.split(data_set[:,2]):
